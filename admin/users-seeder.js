  ...
  */
  const createUsers = async (n = 20) => {
    ...
    for (let i=0; i < n;i++) {
      ...
      promises.push(await createUser(faker.internet.email(faker.name.firstName(gender), faker.name.lastName(gender)), 'w84pgmGent', faker.internet.displayName, `https://picsum.photos/200`));
    }
